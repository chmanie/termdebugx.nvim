...
" Program output command (shown in the "program" window)
if !exists('g:termdebugger_program')
  let g:termdebugger_program = 'tail -f /dev/null;#gdb program'
...
func s:StartDebug_term(dict)
  ...
  let s:pty_job_id = termopen(g:termdebugger_program)
  if s:pty_job_id == 0
...
func s:StartDebug_prompt(dict)
  ...
  if has('win32')
    ...
    execute 'new'
    wincmd x | wincmd j
    belowright let s:pty_job_id = termopen(g:termdebugger_program)
    if s:pty_job_id == 0
      echoerr 'invalid argument (or job table is full) while opening terminal window'
      return
    elseif s:pty_job_id == -1
